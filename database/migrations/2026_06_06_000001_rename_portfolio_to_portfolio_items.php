...
return new class extends Migration
{
    ...
    {
        // Step 1: Rename table from 'portfolio_items' to 'portfolio'
        ...
        if (Schema::hasTable('portfolio')) {
            Schema::table('portfolio', function (Blueprint $table) {
                // Basic columns
                if (!Schema::hasColumn('portfolio', 'id')) {
                    $table->id()->first();
                }
                if (!Schema::hasColumn('portfolio', 'category_id')) {
                    $table->unsignedBigInteger('category_id')->nullable()->after('id');
                }
                if (!Schema::hasColumn('portfolio', 'title')) {
                ...
                if (!Schema::hasColumn('portfolio', 'client')) {
                    $table->string('client')->nullable()->after('year');
                }
                if (!Schema::hasColumn('portfolio', 'services')) {
                    $table->string('services')->nullable()->after('client');
                }
                if (!Schema::hasColumn('portfolio', 'project')) {
                ...
                if (!Schema::hasColumn('portfolio', 'description')) {
                    $table->longText('description')->nullable()->after('short_description');
                }
                if (!Schema::hasColumn('portfolio', 'summary')) {
                    $table->text('summary')->nullable()->after('description');
                }
                
                // SEO columns
                if (!Schema::hasColumn('portfolio', 'meta_title')) {
                    $table->string('meta_title')->nullable()->after('summary');
                }
                if (!Schema::hasColumn('portfolio', 'og_title')) {
                ...
                if (!Schema::hasColumn('portfolio', 'image_alt')) {
                ...
                if (!Schema::hasColumn('portfolio', 'og_description')) {
                ...
                // Timestamps
                if (!Schema::hasColumn('portfolio', 'created_at')) {
                    $table->timestamps();
                }
            });

            // Update status column type if it's integer to string
            $columnType = DB::select("SHOW COLUMNS FROM portfolio WHERE Field = 'status'");
            if (!empty($columnType) && strpos($columnType[0]->Type, 'int') !== false) {
                DB::statement("ALTER TABLE portfolio MODIFY COLUMN status VARCHAR(255) DEFAULT 'Active'");
            }
        }
    }

...
// ── Hero images — Unsplash CDN (free, high quality, no attribution needed for display)
// Download these to /public/images/hero/ for production use:
//   service-hero-1.jpg → https://images.unsplash.com/photo-1555066931-4365d14bab8c?w=1600&q=85
//   service-hero-2.jpg → https://images.unsplash.com/photo-1593720213428-28a5b9e94613?w=1600&q=85
//
// For now, using Unsplash URLs directly. Replace with local paths once downloaded.

const HERO_IMAGES = [
  '/images/hero/service-hero-1.jpg',
  '/images/hero/service-hero-2.jpg',
];

// Fallback Unsplash URLs (used if local images are missing — swap out in production)
const HERO_IMAGES_FALLBACK = [
  'https://images.unsplash.com/photo-1555066931-4365d14bab8c?w=1600&q=85',
  'https://images.unsplash.com/photo-1593720213428-28a5b9e94613?w=1600&q=85',
];
...
export default function ServicesPage({ services = [] }) {
  ...
  // Try local first, fallback to Unsplash CDN
  const [imgSrcs, setImgSrcs] = useState(HERO_IMAGES);

  const handleImgError = (idx) => {
    setImgSrcs(prev => {
      const next = [...prev];
      next[idx] = HERO_IMAGES_FALLBACK[idx];
      return next;
    });
  };
    ...
    <>
      <main className="srv-root">
        ...
        <section className="srv-hero">
          ...
          {/* ── SLIDER IMAGES (background layer) ── */}
          <div className="srv-hero-slider" aria-hidden="true">
            {imgSrcs.map((src, idx) => (
              <div
                key={idx}
                className={`srv-hero-slide${currentSlide === idx ? ' active' : ''}`}
              >
                <img
                  src={src}
                  alt=""
          ...
          <div className="srv-hero-dots-nav" aria-hidden="true">
            {HERO_IMAGES.map((_, idx) => (
              <button
                key={idx}
                className={`srv-dot${currentSlide === idx ? ' active' : ''}`}

...
export default function AdminUsersEdit({ user }) {
    ...
    return (
        <AdminLayout title="Edit User">
            ...
            <div className="page-header">
                ...
                <h2 style={{ fontSize: '1.1rem', fontWeight: 700, color: '#0f172a' }}>Edit User</h2>
            </div>
                ...
                <form onSubmit={handleSubmit}>
                    <div className="form-row">
                        <div className="form-group">
                            ...
                            <input className={`form-input${errors.name ? ' err' : ''}`} value={data.name} onChange={e => setData('name', e.target.value)} />
                            {errors.name && <p className="form-error">{errors.name}</p>}
                        </div>
                        <div className="form-group">
                            <label className="form-label">Email *</label>
                            <input type="email" className={`form-input${errors.email ? ' err' : ''}`} value={data.email} onChange={e => setData('email', e.target.value)} />
                    ...
                    <div className="form-row">
                        <div className="form-group">
                            ...
                            <input type="password" className={`form-input${errors.password ? ' err' : ''}`} value={data.password} onChange={e => setData('password', e.target.value)} placeholder="Leave blank to keep current" />
                            {errors.password && <p className="form-error">{errors.password}</p>}
                            <p className="hint">Leave blank to keep current password</p>
                        </div>
                        <div className="form-group">
                            <label className="form-label">Confirm Password</label>
                            <input type="password" className="form-input" value={data.password_confirmation} onChange={e => setData('password_confirmation', e.target.value)} placeholder="Repeat new password" />
                        </div>
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: '0.75rem', marginTop: '0.5rem', paddingTop: '1rem', borderTop: '1px solid #f1f5f9' }}>
                        <button type="submit" className="btn-primary" disabled={processing}>{processing ? 'Saving...' : 'Update User'}</button>

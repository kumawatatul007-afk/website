...
export default function AdminPortfolioCreate({ categories = [] }) {
    ...
    const handleFileChange = (e) => {
        ...
        if (file) {
            ...
            // Show preview (but don't save base64 to data.image)
            const reader = new FileReader();
            reader.onloadend = () => {
                setImagePreview(reader.result);
            };
            reader.readAsDataURL(file);
            // Clear the text input when file is selected
            setData('image', '');
        }
    };
    ...
    return (
        <AdminLayout title="Create Portfolio Item">
            <style>{`
                ...
                .pf-upload-box { position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:.75rem;padding:1.5rem;border:2px dashed #cbd5e1;border-radius:12px;background:#f8fafc;cursor:pointer;transition:all .2s; }
                .pf-upload-box:hover { border-color:#3b82f6;background:#eff6ff; }
                .pf-upload-box input { position:absolute;inset:0;opacity:0;cursor:pointer; }
                .pf-upload-icon { width:40px;height:40px;border-radius:10px;background:linear-gradient(135deg,#dbeafe,#bfdbfe);color:#2563eb;display:flex;align-items:center;justify-content:center; }
                .pf-upload-text { font-size:.875rem;font-weight:600;color:#475569; }
                .pf-upload-hint { font-size:.75rem;color:#94a3b8; }
                                ...
                                <div className="pf-card-body">
                                    ...
                                    {!imagePreview && !data.image && (
                                        <div className="pf-upload-box">
                                            ...
                                            <div className="pf-upload-text">Click to upload image</div>
                                            <div className="pf-upload-hint">PNG, JPG, GIF, WEBP up to 5MB</div>
                                        </div>
                                    )}

                                    {/* Preview and manual input */}
                                    {(imagePreview || data.image) && (
                                        <>
                                            <div className="pf-img-preview">
                                                <img src={imagePreview || data.image} alt="preview"
                                    ...
                                    <div style={{ marginTop: '1rem', padding: '.75rem', background: '#f1f5f9', borderRadius: '8px' }}>
                                        <div className="pf-field" style={{ marginTop: 0 }}>
                                            <label className="pf-label">Or enter image URL/filename</label>
                                            <input style={inp('image')} value={typeof data.image === 'string' ? data.image : ''}
                                                onChange={e => { setData('image', e.target.value); setData('image_file', null); setImagePreview(null); }}
                                                placeholder="e.g. project.jpg or https://…" />
                                            <span className="pf-hint">Filename from uploads or a full URL</span>
                                        </div>
                                    </div>
                                    
                                    {errors.image && <span className="pf-error" style={{ marginTop: '.5rem', display: 'block' }}>{errors.image}</span>}
                                    {errors.image_file && <span className="pf-error" style={{ marginTop: '.5rem', display: 'block' }}>{errors.image_file}</span>}
                                </div>
                            </div>
